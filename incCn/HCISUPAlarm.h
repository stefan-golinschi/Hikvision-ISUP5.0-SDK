...
#define EHOME_ALARM_QRCODE                20  //二维码报警上传
#define EHOME_ALARM_FACETEMP              21  //人脸测温报警上传
...
#define MAX_SUBSYSTEM_LEN           64   //子系统个数最大值
#define MAX_VIDEO_NUM               5    //最大视频数量
...
typedef struct tagNET_EHOME_CID_INFO_INTERNAL_EX
{
    ...
    BYTE  byIntercomTriggerEnabled; //对讲触发使能，0-未触发，1-触发警号对讲
    BYTE  byMultiVideoUrl;  //是否有多个视频URL，0-单个视频URL，1-多个视频URL
    BYTE  byRes;
    ...
    BYTE  byLinkageSubSystem[MAX_SUBSYSTEM_LEN];  //关联的子系统
    float  fTemperatureValue; //温度值，单位摄氏度(℃)
#if (defined(OS_WINDOWS64) || defined(OS_POSIX64))//64位系统下指针为8字节
    void*  pVideoInfo;  //byMultiVideoUrl为1时有效，指向NET_EHOME_CID_INFO_INTERNAL_VIDEOINFO结构体
#else
    void*  pVideoInfo; //同上
    BYTE     byRes2[4];
#endif
...
{
    char byVideoURL[MAX_VIDEO_NUM][MAX_URL_LEN];//视频URL
    BYTE  byRes[512];
