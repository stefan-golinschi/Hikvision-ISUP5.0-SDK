...
typedef struct tagNET_EHOME_VOICE_TALK_IN
{
    ...
    BYTE  byBroadLevel;//语音广播优先级标识,0~15优先级从低到高,当存在byBroadcast为1时,0标识最低优先级。当存在byBroadcast为0时，本节点无意义为保留字节
    BYTE  byBroadVolume; //语音广播音量,0~15音量从低到高,当存在byBroadcast为1时,0标识最低音量。当存在byBroadcast为0时，本节点无意义为保留字节
...
#define NP_FVY100                   14007 //视频烟感
#define NP_FVW100                   14008 //安消智能相机
#define NP_FVG100                   14009 //安消联动相机
#define NP_FVY300                   14010 //经济型视频烟感
#define NP_FVR212_P                 14011 //三合一火灾探测器
#define NP_FVR213_P                 14012 //三合一火灾探测器(自研)
/**********************设备类型值定义 end**********************/
...
{
    int     iChannel;     //通道号 
    DWORD   dwStreamType; //码流类型，0-主码流，1-子码流, 2-第三码流,3-语音监听（ISUP2.0/4.0使用, ISUP5.0空掉）,4-虚拟码流(NPQ使用)复用第四码流，5-第五码流，..，10-第十码流
...
typedef struct tagNET_EHOME_PLAYBACK_INFO_IN
{
    ...
    union
    {
        ...
        struct
        {
            ...
            BYTE   byStreamType;                    //1 - 主码流，2 - 子码流, 0为默认设备有什么流推送什么流
        }struPlayBackbyName;
        struct 
        {
            ...
            BYTE    byTimeLapse;            //延时摄影回放，0-否，1-是
        }struPlayBackbyTime;
    }unionPlayBackMode;
    NET_EHOME_IPADDRESS struStreamSever;    //流媒体地址
...
typedef enum tagNET_ISUP_CMS_LF_CB_TYPE
{
    ...
    DATETYPE_DOWNLOAD_END = 6,      //文件下载结束        ClassType(pOutBuffer) = HPR_UINT32 *状态值，成功=0，其他值见错误码
...
//dwDataType类型以及pOutBuffer转换见NET_ISUP_CMS_LF_CB_TYPE，pInBuffer暂无用，保留
...
#define INTERACT_BY_BUF  0  //以内存数据方式与SDK交互，文件上传交互见接口NET_ISUP_CMS_LFUpload，文件下载交互见回调接口LF_CB中类型DATETYPE_DOWNLOAD_RSP
//#define INTERACT_BY_FILE 1  //以设置文件路径方式，通过SDK自己读取文件内容
...
typedef enum tagNET_ISUP_CMS_LF_SET_BUSINESS_PARAM
{
    ...
    //DATETYPE_INTERACT_BY_FILE,         //按文件路径交互数据，需要设置到文件名 路径最大长度不能超过260，ClassType(pBuffer) = LPNET_ISUP_CMS_LF_PARAM, buf为文件路径
}NET_ISUP_CMS_LF_SET_BUSINESS_PARAM;

typedef struct tagNET_ISUP_CMS_LF_CONFIG_PARAM
{
    char*          pFileId;				//发送的FileId = sha256（filename + filesize）, 断点续传使用，此时要填入对应的firsttag、lasttag, 最大512
    DWORD          dwFileIdLen;			//
    ...
    DWORD          dwSequence;          //[out]Sequence为当次数据传输的标记，如下载数据包比较大时，分多包次发送，每包Sequence值相同
    BYTE           byRes[128];
